...
7. E2E tests use **Playwright** (`tests/e2e/`), run via `pnpm run test:e2e`. Unit tests use **Node.js test runner** (`tests/unit/`), run via `pnpm run test:unit`. Source code under `src/` is **TypeScript** (`.ts`/`.tsx`); the `open-sse/` workspace remains JavaScript (`.js`).
...
- Build from source: `pnpm run build`
